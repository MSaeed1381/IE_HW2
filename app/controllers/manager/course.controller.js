...
export default class CourseController {
    static async create(req, res) {
        ...
        if (courseType === "approved"){
            ...
            try {
                const {courseName, prerequisites, corequisites, unit} = req.body;
                const course = new ApprovedCourse({
                    courseName, prerequisites, corequisites, unit
                });
                const data = await course.save(course);
                res.status(201).json(createResponse(true, "Course Created Successfully!", data));
            } catch (err) {
            ...
            try {
                const {courseName, prerequisites, corequisites, unit,  classDate,
                    examDate, examLocation, courseProfessor, capacity, educationSemester} = req.body;

                const course = new SemesterCourses({
                    courseName, prerequisites, corequisites, unit,  classDate,
                    examDate, examLocation, courseProfessor, capacity, educationSemester
                });
    ...
    static async update(req, res){
        ...
        try {
            ...
            const data = await Course.findByIdAndUpdate(id, req.body, {useFindAndModify: true});
            if (data)
    ...
    static async delete(req, res){
        ...
        try {
            ...
            if (data)
                return res.status(200).json(createResponse(true, "Course Deleted Successfully"));
    ...
    static async getAllCourses(req, res){
        try{
            ...
            return res.status(200).json(createResponse(true, "get all Courses", data));
        } catch (err) {
