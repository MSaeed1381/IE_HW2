...
export default class EducationController {
    static async create(req, res) {
        ...
        try {
            ...
            res.status(500).json(createResponse(false,
                err.message || "Some error occurred while creating the Education Manager."));
        }
    }
    ...
    static async update(req, res){
        ...
        try {
            ...
            const data = await EducationManager.findByIdAndUpdate(id, req.body, {useFindAndModify: true});
            if (data)
                return res.status(200).json(createResponse(true, "Education Manager Updated Successfully"));
            return res.status(404).json(createResponse(false, "Education Manager not found"));
        }catch (err) {
            return res.status(500).json(false,
                err.message || "Some error occurred while updating the Education Manager.")
        }
    }
    ...
    static async delete(req, res){
        ...
        try {
            ...
            if (data)
                return res.status(200).json(createResponse(true, "Education Manager Deleted Successfully"));
            return res.status(404).json(createResponse(false, "Education Manager not found"));
        } catch(err){
            res.status(500).json(createResponse(false,
                err.message || "Some error occurred while deleting the Education Manager."));
        }
    }
    static async getAllEducationManagers(req, res){
        try{
            const data = await EducationManager.find();
            return res.status(200).json(createResponse(true, "get all Education Managers", data));
        } catch (err) {
        ...
        try {
            ...
            if (data)
                return res.status(200).json(createResponse(true,
                    `get Education Manager with id ${id}.`, data));
