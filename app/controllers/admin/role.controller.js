...
export default class RoleController {
    static async create(req, res) {
        ...
        try {
            ...
            res.status(201).json(createResponse(true, "Role Created Successfully!", data));
        } catch (err) {
    ...
    static async update(req, res){
        ...
        try {
            ...
            const data = await Role.findByIdAndUpdate(id, req.body, {useFindAndModify: true});
            if (data)
                return res.status(200).json(createResponse(true, "Role Updated Successfully"));
    ...
    static async delete(req, res){
        ...
        try {
            ...
            if (data)
                return res.status(200).json(createResponse(true, "Role Deleted Successfully"));
            return res.status(404).json(createResponse(false, "Role not found"));
        } catch(err){
            res.status(500).json(createResponse(false,
    ...
    static async getAllRoles(req, res){
        try{
            ...
            return res.status(200).json(createResponse(true, "get all roles", data));
        } catch (err) {
