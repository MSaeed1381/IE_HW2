...
export default class LoginController {
    static async login(req, res){
        if (!existAllParams(requiredLoginParams, req.body))
            return res.status(400).json(createResponse(false, "Content is incomplete!"));
        ...
        if (!user)
            return res.status(404).json(createResponse(false, `User with id ${user_id} not found`));

        const success = await compare(password, user.password_hash);
        if (!success)
            return res.status(400).json(createResponse(false, `password is not correct`));

        const accessToken = jwt.sign(user_id, process.env.ACCESS_TOKEN_SECRET);
        return res.status(200).json(createResponse(true, `Hi ${user.full_name}`, accessToken));
